...
const TemplatePdfDeliveryOrder = async ({ data }) => {
  try {
    // ================ DATA EXTRACTION ================
    const {
      status,
    ...
    const renderProductRow = (product, yPosition) => {
      const { productName, rackName, unitName, quantity } = product;
...
module.exports = { TemplatePdfDeliveryOrder };
...
function buildQueryOptions(query, options = {}) {
  const { searchFields = [], statusField = null, dateField = 'createdAt', enableDate = true, additionalWhere = {} } = options;
  ...
  if (enableDate) {
    if (query?.dateFrom || query?.dateTo) {
      ...
      const formattedDate = moment(query.date, "DD-MM-YYYY").format("YYYY-MM-DD");
